...
use errors::{INKWELL_FUNC_FAILED, IR_FIND_PRIMITIVE_TYPE, IR_INVALID_INT_COMP_VAL, IR_TYPE_BOOL, errs::{BaseResult, base::BaseError}};
...
pub fn make_bool_xor(ctx: &IRContext, b: IRValueRef) -> BaseResult<IRValue> {
	...
	let inkwell = match val.obtain_as_bool() {
		Some(v) => v,
		None => return Err(BaseError::err(IR_TYPE_BOOL!().to_string()))
	};

	let bool_t = match ctx.type_storage.get(BOOL_TYPE_HASH) {
		Some(v) => v,
		None => return Err(BaseError::critical(IR_FIND_PRIMITIVE_TYPE!().to_string()))
	};
	...
	let xor_v = match ctx.builder.build_xor(inkwell.inner, one, "xor_") {
		Ok(v) => v,
		Err(e) => return Err(BaseError::critical(format!(INKWELL_FUNC_FAILED!(), "build_xor", e)))
	};

	return Ok(IRValue::new(OwnedValueEnum::new(&ctx.inkwell_ctx, xor_v.into()), bool_t));
}

pub fn make_bool_cmp_int(ctx: &IRContext, a: IRValueRef, b: IRValueRef, comp: ComparingOperator) -> BaseResult<IRValue> {
	...
	let a_int = match a_val.obtain_as_int(ctx, t.clone()) {
		Some(v) => v,
		None => return Err(BaseError::err(format!(IR_INVALID_INT_COMP_VAL!(), "a")))
	};

	let b_int = match b_val.obtain_as_int(ctx, t.clone()) {
		Some(v) => v,
		None => return Err(BaseError::err(format!(IR_INVALID_INT_COMP_VAL!(), "b")))
	};
	...
	let cmp = match ctx.builder.build_int_compare(predicate, *a_int, *b_int, "_cmp") {
		Ok(v) => v,
		Err(e) => return Err(BaseError::critical(format!(INKWELL_FUNC_FAILED!(), "build_int_compare", e)))
	};
