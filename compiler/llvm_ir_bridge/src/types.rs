...
impl LLVMTypeStorage {
    ...
    pub fn convert_raw(&mut self, base: RawType) -> LLVMTypeEnum {
        ...
        let conv: BasicTypeEnum = match &base {
            RawType::Integer(a, _) => self
                .ctxref
                .custom_width_int_type(NonZero::new(*a as u32).unwrap())
                .unwrap()
                .into(),

            RawType::Floating(a, _) => match a {
            ...
            RawType::FixedPoint(a, b, _) => {
                ...
                self.ctxref
                    .custom_width_int_type(NonZero::new(sum as u32).unwrap())
                    .unwrap()
                    .into()
            }
            ...
            RawType::Pointer => self.ctxref.ptr_type(AddressSpace::from(0u16)).into(),

            _ => panic!("cannot convert to LLVM type!"),
        };

        let l =
            LLVMTypeEnum::new(unsafe { transmute::<BasicTypeEnum, BasicTypeEnum<'static>>(conv) });
