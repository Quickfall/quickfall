...
use inkwell::{basic_block::BasicBlock, context::Context, types::{BasicMetadataTypeEnum, BasicTypeEnum, IntType, PointerType}, values::{BasicValueEnum, FunctionValue}};
...
pub fn get_block_name() -> String {
	let s = rand::thread_rng().sample_iter(&Alphanumeric).take(24).map(char::from).collect();
