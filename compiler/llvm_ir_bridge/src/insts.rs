...
pub fn bridge_llvm_instruction(instruction: MIRBlockHeldInstruction, func: usize, bridge: &mut LLVMBridgeContext, mir: &MIRContext) -> BaseResult<Option<LLVMBasicValue>> {
	...
	let res: Option<BasicValueEnum<'static>> = match MIRInstruction::from(instruction.clone().into()) {
		...
		MIRInstruction::BitwiseNot { val } => {
			...
			let res: IntValue<'static> = llvm_to_base!(bridge.builder.build_not(v.into_int_value(), "e"));
		...
		MIRInstruction::StaticStringConstant { raw } => {
			...
			let mut vals: Vec<IntValue> = bytes.iter().map(|b| byte_type.const_int(*b as u64, false)).collect();
		...
		MIRInstruction::ConditionalBranch { cond, if_branch, else_branch } => {
			let cond: BaseMIRValue = MIRIntValue::into(cond);

			let cond = bridge.values[&cond.get_ssa_index()].clone().into_int_value();

			let if_branch = bridge.blocks[&if_branch].clone();
			let else_branch = bridge.blocks[&else_branch].clone();

			llvm_to_base_returnless!(bridge.builder.build_conditional_branch(cond, if_branch.inner, else_branch.inner));
		...
		MIRInstruction::Phi { choices } => {
			...
			for choice in choices {
				...
				let value = bridge.values[&choice.1.get_ssa_index()].inner.as_basic_value_enum();
		...
		MIRInstruction::Select { cond, if_val, else_val } => {
			let cond: BaseMIRValue = MIRIntValue::into(cond);
		...
		MIRInstruction::FieldPointer { val, field } => {
			...
			let res = llvm_to_base!(bridge.builder.build_struct_gep(struct_type, ptr_val.into_pointer_value(), field as u32, ""));
