...
impl TypeStorage {
	pub fn new() -> BaseResult<Self> {
		...
		storage.register_type(STATIC_STR, BaseType::StaticStr)?;
	...
	pub fn get_complex(&self, hash: u64) -> BaseResult<ComplexType> {
		let base = self.get_type(hash)?.1.clone();
		let concrete = ConcreteType { base, pointer: false, pointer_array: false, type_params: vec![], size_params: vec![] };

		return Ok(ComplexType::Concrete(concrete))
	}
