    ...
    pub fn new() -> Self {
        MIRContext {
            ...
            writer: InstructionWriterPosition {
                curr_block: 0,
                curr_inst: BlockPosition::END,
            },
            block_to_func: HashMap::new(),
        }
    }
    ...
    pub fn append_inst(&mut self, inst: MIRInstruction) -> InstructionValue {
        if inst.has_return(self) {
            ...
            if !inst.should_hint() {
                ...
                self.blocks[self.writer.curr_block].append(
                    MIRBlockHeldInstruction::Valued(inst.clone(), hint_ind),
                    &self.writer.curr_inst,
                );
        ...
        self.blocks[self.writer.curr_block].append(
            MIRBlockHeldInstruction::Valueless(inst.clone()),
            &self.writer.curr_inst,
        );
