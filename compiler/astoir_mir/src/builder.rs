...
pub fn build_stack_alloc(ctx: &MIRContext, block: &mut MIRBlock, size: usize, t: CompactedType) -> BaseResult<MIRPointerValue> {
...
pub fn build_load(ctx: &MIRContext, block: &mut MIRBlock, ptr: MIRPointerValue) -> BaseResult<BaseMIRValue> {
	let res = block.append(ctx, MIRInstruction::Load { value: ptr }).get()?;

	return Ok(res);
}

pub fn build_store(ctx: &MIRContext, block: &mut MIRBlock, ptr: MIRPointerValue, val: BaseMIRValue) -> BaseResult<bool> {
...
pub fn build_downcast_int(ctx: &MIRContext, block: &mut MIRBlock, val: MIRIntValue, size: usize) -> BaseResult<MIRIntValue> {
	if val.size <= size {
...
pub fn build_upcast_int(ctx: &MIRContext, block: &mut MIRBlock, val: MIRIntValue, size: usize) -> BaseResult<MIRIntValue> {
	if val.size >= size {
...
pub fn build_downcast_float(ctx: &MIRContext, block: &mut MIRBlock, val: MIRFloatValue, exponent: usize, fraction: usize) -> BaseResult<MIRFloatValue> {
...
pub fn build_upcast_float(ctx: &MIRContext, block: &mut MIRBlock, val: MIRFloatValue, exponent: usize, fraction: usize) -> BaseResult<MIRFloatValue> {
	if val.exponent + val.fraction >= exponent + fraction {
...
pub fn build_int_add(ctx: &MIRContext, block: &mut MIRBlock, left: MIRIntValue, right: MIRIntValue, signed: bool) -> BaseResult<MIRIntValue> {
...
pub fn build_int_sub(ctx: &MIRContext, block: &mut MIRBlock, left: MIRIntValue, right: MIRIntValue, signed: bool) -> BaseResult<MIRIntValue> {
	if left.size != right.size {
...
pub fn build_int_mul(ctx: &MIRContext, block: &mut MIRBlock, left: MIRIntValue, right: MIRIntValue, signed: bool) -> BaseResult<MIRIntValue> {
	if left.size != right.size {
...
pub fn build_int_div(ctx: &MIRContext, block: &mut MIRBlock, left: MIRIntValue, right: MIRIntValue, signed: bool) -> BaseResult<MIRIntValue> {
	if left.size != right.size {
...
pub fn build_int_neg(ctx: &MIRContext, block: &mut MIRBlock, val: MIRIntValue) -> BaseResult<MIRIntValue> {
	let res = block.append(ctx, MIRInstruction::IntegerNeg { val }).get()?;

	return res.as_int();
}

pub fn build_float_add(ctx: &MIRContext, block: &mut MIRBlock, left: MIRFloatValue, right: MIRFloatValue, signed: bool) -> BaseResult<MIRFloatValue> {
...
pub fn build_float_sub(ctx: &MIRContext, block: &mut MIRBlock, left: MIRFloatValue, right: MIRFloatValue, signed: bool) -> BaseResult<MIRFloatValue> {
	if left.exponent != right.exponent || left.fraction != right.fraction {
...
pub fn build_float_mul(ctx: &MIRContext, block: &mut MIRBlock, left: MIRFloatValue, right: MIRFloatValue, signed: bool) -> BaseResult<MIRFloatValue> {
	if left.exponent != right.exponent || left.fraction != right.fraction {
...
pub fn build_float_div(ctx: &MIRContext, block: &mut MIRBlock, left: MIRFloatValue, right: MIRFloatValue, signed: bool) -> BaseResult<MIRFloatValue> {
	if left.exponent != right.exponent || left.fraction != right.fraction {
...
pub fn build_float_neg(ctx: &MIRContext, block: &mut MIRBlock, val: MIRFloatValue) -> BaseResult<MIRFloatValue> {
	let res = block.append(ctx, MIRInstruction::FloatNeg { val }).get()?;

	return res.as_float();
}

pub fn build_bitwise_and(ctx: &MIRContext, block: &mut MIRBlock, a: MIRIntValue, b: MIRIntValue) -> BaseResult<MIRIntValue> {
...
pub fn build_bitwise_or(ctx: &MIRContext, block: &mut MIRBlock, a: MIRIntValue, b: MIRIntValue) -> BaseResult<MIRIntValue> {
	if a.size != 1 || b.size != 1 {
...
pub fn build_bitwise_xor(ctx: &MIRContext, block: &mut MIRBlock, a: MIRIntValue, b: MIRIntValue) -> BaseResult<MIRIntValue> {
	if a.size != 1 || b.size != 1 {
...
pub fn build_bitwise_not(ctx: &MIRContext, block: &mut MIRBlock, a: MIRIntValue) -> BaseResult<MIRIntValue> {
	if a.size != 1 {
...
pub fn build_shift_left(ctx: &MIRContext, block: &mut MIRBlock, val: MIRIntValue, shift: MIRIntValue) -> BaseResult<MIRIntValue> {
	let res = block.append(ctx, MIRInstruction::ShiftLeft { a: val, shift }).get()?;

	return res.as_int();
}

pub fn build_shift_right(ctx: &MIRContext, block: &mut MIRBlock, val: MIRIntValue, shift: MIRIntValue) -> BaseResult<MIRIntValue> {
	let res = block.append(ctx, MIRInstruction::ShiftRight { a: val, shift }).get()?;

	return res.as_int();
}

pub fn build_comp_eq(ctx: &MIRContext, block: &mut MIRBlock, a: MIRIntValue, b: MIRIntValue) -> BaseResult<MIRIntValue> {
	if a.size != b.size {
...
pub fn build_comp_neg(ctx: &MIRContext, block: &mut MIRBlock, a: MIRIntValue, b: MIRIntValue) -> BaseResult<MIRIntValue> {
	if a.size != b.size {
...
pub fn build_comp_lt(ctx: &MIRContext, block: &mut MIRBlock, a: MIRIntValue, b: MIRIntValue) -> BaseResult<MIRIntValue> {
	if a.size != b.size {
...
pub fn build_comp_le(ctx: &MIRContext, block: &mut MIRBlock, a: MIRIntValue, b: MIRIntValue) -> BaseResult<MIRIntValue> {
	if a.size != b.size {
...
pub fn build_comp_gt(ctx: &MIRContext, block: &mut MIRBlock, a: MIRIntValue, b: MIRIntValue) -> BaseResult<MIRIntValue> {
	if a.size != b.size {
...
pub fn build_comp_ge(ctx: &MIRContext, block: &mut MIRBlock, a: MIRIntValue, b: MIRIntValue) -> BaseResult<MIRIntValue> {
	if a.size != b.size {
...
pub fn build_return(ctx: &MIRContext, block: &mut MIRBlock, val: BaseMIRValue) -> BaseResult<bool> {
	block.append(ctx, MIRInstruction::Return { val });

	Ok(true)
}

pub fn build_unconditional_branch(ctx: &MIRContext, block: &mut MIRBlock, func: &MIRFunction, branch: MIRBlockReference) -> BaseResult<bool> {
...
pub fn build_conditional_branch(ctx: &MIRContext, block: &mut MIRBlock, func: &MIRFunction, condition: MIRIntValue, if_branch: MIRBlockReference, else_branch: MIRBlockReference) -> BaseResult<bool> {
	if condition.size != 1 {
...
pub fn build_phi(ctx: &MIRContext, block: &mut MIRBlock, func: &MIRFunction, choices: Vec<(MIRBlockReference, BaseMIRValue)>) -> BaseResult<BaseMIRValue> {
...
pub fn build_select(ctx: &MIRContext, block: &mut MIRBlock, condition: MIRIntValue, if_val: BaseMIRValue, else_val: BaseMIRValue) -> BaseResult<BaseMIRValue> {
	if condition.size != 1 {
...
pub fn build_field_pointer(ctx: &MIRContext, block: &mut MIRBlock, ptr: MIRPointerValue, field: usize) -> BaseResult<MIRPointerValue> {
...
pub fn build_index_pointer(ctx: &MIRContext, block: &mut MIRBlock, val: MIRPointerValue, index: usize) -> BaseResult<bool> {
	block.append(ctx, MIRInstruction::IndexPointer { val, index });

	Ok(true)
}

pub fn build_marker_era_drop(ctx: &MIRContext, block: &mut MIRBlock, val: BaseMIRValue) -> BaseResult<bool> {
	block.append(ctx, MIRInstruction::MarkerEraDrop { value: val });

	Ok(true)
}

pub fn build_signed_int_const(ctx: &MIRContext, block: &mut MIRBlock, raw: i128, bitsize: usize) -> BaseResult<MIRIntValue> {
	let res = block.append(ctx, MIRInstruction::IntegerSignedConstant { raw, bitsize }).get()?;

	return res.as_int();
}

pub fn build_unsigned_int_const(ctx: &MIRContext, block: &mut MIRBlock, raw: u128, bitsize: usize) -> BaseResult<MIRIntValue> {
	let res = block.append(ctx, MIRInstruction::IntegerUnsignedConstant { raw, bitsize }).get()?;

	return res.as_int();
}

pub fn build_signed_float_const(ctx: &MIRContext, block: &mut MIRBlock, raw: f64, exponent: usize, fraction: usize) -> BaseResult<MIRFloatValue> {
	let res = block.append(ctx, MIRInstruction::FloatSignedConstant { raw, exponent, fraction }).get()?;

	return res.as_float();
}

pub fn build_unsigned_float_const(ctx: &MIRContext, block: &mut MIRBlock, raw: f64, exponent: usize, fraction: usize) -> BaseResult<MIRFloatValue> {
	let res = block.append(ctx, MIRInstruction::FloatUnsignedConstant { raw, exponent, fraction }).get()?;

	return res.as_float();
}

pub fn build_signed_fixed_const(ctx: &MIRContext, block: &mut MIRBlock, raw: f64, number: usize, fraction: usize) -> BaseResult<MIRIntValue> {
	let res = block.append(ctx, MIRInstruction::FixedSignedConstant { raw, number, fraction }).get()?;

	return res.as_int();
}

pub fn build_unsigned_fixed_const(ctx: &MIRContext, block: &mut MIRBlock, raw: f64, number: usize, fraction: usize) -> BaseResult<MIRIntValue> {
	let res = block.append(ctx, MIRInstruction::FixedUnsignedConstant { raw, number, fraction }).get()?;

	return res.as_int();
}

pub fn build_static_string_const(ctx: &MIRContext, block: &mut MIRBlock, raw: String) -> BaseResult<MIRPointerValue> {
	let res = block.append(ctx, MIRInstruction::StaticStringConstant { raw }).get()?;

	return res.as_ptr();
}

pub fn build_call(ctx: &MIRContext, block: &mut MIRBlock, func: &MIRFunction, ind: usize, args: Vec<BaseMIRValue>) -> BaseResult<BaseMIRValue> {
