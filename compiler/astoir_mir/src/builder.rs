...
use crate::{
    ...
    vals::{
        arrays::MIRArrayValue, base::BaseMIRValue, float::MIRFloatValue, int::MIRIntValue,
        ptr::MIRPointerValue, structs::MIRStructValue,
    },
};
...
pub fn build_stack_alloc(
    ...
    size: usize,
    t: Type,
) -> DiagnosticResult<MIRPointerValue> {
    let res = ctx
        .append_inst(MIRInstruction::StackAlloc {
            alloc_size: size,
            t: t.clone(),
        })
        .get()?;
...
pub fn build_downcast_int(
    ...
    let res = ctx
        .append_inst(MIRInstruction::DowncastInteger { val, size })
        .get()?;

    return res.as_int();
}

pub fn build_upcast_int(
    ctx: &mut MIRContext,
    val: MIRIntValue,
    size: usize,
) -> DiagnosticResult<MIRIntValue> {
    ...
    let res = ctx
        .append_inst(MIRInstruction::UpcastInteger { val, size })
        .get()?;

    return res.as_int();
}

pub fn build_downcast_float(
    ctx: &mut MIRContext,
    val: MIRFloatValue,
    size: usize,
) -> DiagnosticResult<MIRFloatValue> {
    if val.size <= size {
        unsure_panic!("tried downcasting on a smaller sized float");
    }

    let res = ctx
        .append_inst(MIRInstruction::DowncastFloat { val, size })
        .get()?;

    return res.as_float();
}

pub fn build_upcast_float(
    ctx: &mut MIRContext,
    val: MIRFloatValue,
    size: usize,
) -> DiagnosticResult<MIRFloatValue> {
    ...
    let res = ctx
        .append_inst(MIRInstruction::UpcastFloat { val, size })
        .get()?;
...
pub fn build_int_add(
    ...
    let res = ctx
        .append_inst(MIRInstruction::IntegerAdd {
            signed,
            fast,
            left,
            right,
...
pub fn build_int_sub(
    ...
    let res = ctx
        .append_inst(MIRInstruction::IntegerSub {
            signed,
            fast,
            left,
            right,
...
pub fn build_int_mul(
    ...
    let res = ctx
        .append_inst(MIRInstruction::IntegerMul {
            signed,
            fast,
            left,
            right,
...
pub fn build_int_div(
    ...
    let res = ctx
        .append_inst(MIRInstruction::IntegerDiv {
            signed,
            fast,
            left,
            right,
...
pub fn build_int_mod(
    ...
    let res = ctx
        .append_inst(MIRInstruction::IntegerMod {
            signed,
            fast,
            left,
            right,
...
pub fn build_int_shift_left(
    ctx: &mut MIRContext,
    left: MIRIntValue,
    shift: MIRIntValue,
) -> DiagnosticResult<MIRIntValue> {
    let res = ctx
        .append_inst(MIRInstruction::ShiftLeft { a: left, shift })
        .get()?;

    return res.as_int();
}

pub fn build_int_shift_right(
    ctx: &mut MIRContext,
    left: MIRIntValue,
    shift: MIRIntValue,
) -> DiagnosticResult<MIRIntValue> {
    let res = ctx
        .append_inst(MIRInstruction::ShiftRight { a: left, shift })
        .get()?;
...
pub fn build_float_add(
    ...
    let res = ctx
        .append_inst(MIRInstruction::FloatAdd {
            signed,
            fast,
            left,
            right,
...
pub fn build_float_sub(
    ...
    let res = ctx
        .append_inst(MIRInstruction::FloatSub {
            signed,
            fast,
            left,
            right,
...
pub fn build_float_mul(
    ...
    let res = ctx
        .append_inst(MIRInstruction::FloatMul {
            signed,
            fast,
            left,
            right,
...
pub fn build_float_div(
    ...
    let res = ctx
        .append_inst(MIRInstruction::FloatDiv {
            signed,
            fast,
            left,
            right,
...
pub fn build_float_mod(
    ...
    let res = ctx
        .append_inst(MIRInstruction::FloatMod {
            signed,
            fast,
            left,
            right,
...
pub fn build_bitwise_not(ctx: &mut MIRContext, a: MIRIntValue) -> DiagnosticResult<MIRIntValue> {
    ...
    let res = ctx
        .append_inst(MIRInstruction::BitwiseNot { val: a })
        .get()?;
...
pub fn build_shift_left(
    ...
    let res = ctx
        .append_inst(MIRInstruction::ShiftLeft { a: val, shift })
        .get()?;
...
pub fn build_shift_right(
    ...
    let res = ctx
        .append_inst(MIRInstruction::ShiftRight { a: val, shift })
        .get()?;

    return res.as_int();
}

pub fn build_comp_eq(
    ctx: &mut MIRContext,
    a: MIRIntValue,
    b: MIRIntValue,
) -> DiagnosticResult<MIRIntValue> {
...
pub fn build_conditional_branch(
    ctx: &mut MIRContext,
    condition: MIRIntValue,
    if_branch: MIRBlockReference,
    else_branch: MIRBlockReference,
) -> DiagnosticResult<bool> {
    if condition.size != 1 {
        unsure_panic!("provided cond to build_conditional_branch is not a boolean");
    }

    ctx.append_inst(MIRInstruction::ConditionalBranch {
        cond: condition,
        if_branch,
        else_branch,
    });
...
pub fn build_select(
    ...
    return ctx
        .append_inst(MIRInstruction::Select {
            cond: condition,
            if_val,
            else_val,
        })
...
pub fn build_field_pointer(
    ctx: &mut MIRContext,
    ptr: MIRPointerValue,
    field: usize,
) -> DiagnosticResult<MIRPointerValue> {
    let val = ctx
        .append_inst(MIRInstruction::FieldPointer {
            val: ptr.clone(),
            field,
        })
        .get()?;
    ...
    let pointer_hint = ctx
        .ssa_hints
        .get_hint(base.get_ssa_index())
        .as_pointer()
        .as_generic_lowered();
    let t;
...
pub fn build_index_pointer(
    ctx: &mut MIRContext,
    val: MIRPointerValue,
    index: MIRIntValue,
) -> DiagnosticResult<MIRPointerValue> {
    let res = ctx
        .append_inst(MIRInstruction::IndexPointer {
            val: val.clone(),
            index,
        })
        .get()?;
...
pub fn build_signed_int_const(
    ctx: &mut MIRContext,
    raw: i128,
    bitsize: usize,
) -> DiagnosticResult<MIRIntValue> {
    let res = ctx
        .append_inst(MIRInstruction::IntegerSignedConstant { raw, bitsize })
        .get()?;

    return res.as_int();
}

pub fn build_unsigned_int_const(
    ctx: &mut MIRContext,
    raw: u128,
    bitsize: usize,
) -> DiagnosticResult<MIRIntValue> {
    let res = ctx
        .append_inst(MIRInstruction::IntegerUnsignedConstant { raw, bitsize })
        .get()?;

    return res.as_int();
}

pub fn build_signed_float_const(
    ctx: &mut MIRContext,
    raw: f64,
    size: usize,
) -> DiagnosticResult<MIRFloatValue> {
    let res = ctx
        .append_inst(MIRInstruction::FloatSignedConstant { raw, size })
        .get()?;

    return res.as_float();
}

pub fn build_unsigned_float_const(
    ctx: &mut MIRContext,
    raw: f64,
    size: usize,
) -> DiagnosticResult<MIRFloatValue> {
    let res = ctx
        .append_inst(MIRInstruction::FloatUnsignedConstant { raw, size })
        .get()?;

    return res.as_float();
}

pub fn build_signed_fixed_const(
    ctx: &mut MIRContext,
    raw: f64,
    number: usize,
    fraction: usize,
    ...
    let res = ctx
        .append_inst(MIRInstruction::FixedSignedConstant {
            raw,
            number,
            fraction,
        })
...
pub fn build_unsigned_fixed_const(
    ctx: &mut MIRContext,
    raw: f64,
    number: usize,
    fraction: usize,
    ...
    let res = ctx
        .append_inst(MIRInstruction::FixedUnsignedConstant {
            raw,
            number,
            fraction,
        })
...
pub fn build_static_string_const(
    ctx: &mut MIRContext,
    raw: String,
) -> DiagnosticResult<MIRPointerValue> {
    let res = ctx
        .append_inst(MIRInstruction::StaticStringConstant { raw })
        .get()?;

    return res.as_ptr();
}

pub fn build_static_struct_const(
    ctx: &mut MIRContext,
    struct_type: RawType,
    values: Vec<BaseMIRValue>,
) -> DiagnosticResult<MIRStructValue> {
    let res = ctx
        .append_inst(MIRInstruction::StructInitializerConstant {
            struct_type,
            values,
        })
        .get()?;
...
pub fn build_static_array_const(
    ctx: &mut MIRContext,
    values: Vec<BaseMIRValue>,
) -> DiagnosticResult<MIRArrayValue> {
    let res = ctx
        .append_inst(MIRInstruction::ArrayInitializerConstant { values })
        .get()?;

    return res.as_array();
}

pub fn build_static_array_one_const(
    ctx: &mut MIRContext,
    val: BaseMIRValue,
    size: usize,
) -> DiagnosticResult<MIRArrayValue> {
    let res = ctx
        .append_inst(MIRInstruction::ArrayInitializerConstantSame { size, val })
        .get()?;

    return res.as_array();
}

pub fn build_argument_grab(
    ctx: &mut MIRContext,
    index: usize,
    t: Type,
) -> DiagnosticResult<BaseMIRValue> {
    let res = ctx
        .append_inst(MIRInstruction::FuncArgumentGrab {
            ind: index,
            argtype: t,
        })
        .get()?;
...
pub fn build_call(
    ...
    let res = ctx
        .append_inst(MIRInstruction::Call {
            function: ind,
            arguments: args,
        })
        .val;
...
pub fn build_phi(
    ctx: &mut MIRContext,
    choices: Vec<(MIRBlockReference, BaseMIRValue)>,
) -> DiagnosticResult<BaseMIRValue> {
...
pub fn build_memory_copy_unsafe(
    ctx: &mut MIRContext,
    src: MIRPointerValue,
    dest: MIRPointerValue,
    size: usize,
) {
    ctx.append_inst(MIRInstruction::MemoryCopy {
        src,
        dest,
        sz: size,
    });
