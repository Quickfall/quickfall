...
use diagnostics::{
    DiagnosticResult, DiagnosticSpanOrigin, builders::make_req_type_kind, unsure_panic,
};
...
impl Type {
    ...
    pub fn is_truly_eq(&self, other: &Type) -> bool {
        match (self, other) {
            ...
            (
                Self::Generic(raw_type, type_params, sizes),
                Self::Generic(raw_type2, type_params2, sizes2),
            ) => {
    ...
    pub fn as_generic_lowered_safe<K: DiagnosticSpanOrigin>(
        &self,
        origin: &K,
    ...
    pub fn as_generic_safe<K: DiagnosticSpanOrigin>(
        &self,
        origin: &K,
    ) -> DiagnosticResult<RawType> {
        match self {
    ...
    pub fn get_inner_type(&self) -> Box<Type> {
        match self {
            ...
            _ => {
                panic!(
                    "Error! Compiler tried using get_inner_type on bottom type! Returning bottom type incase!"
                );
            }
        }
    }
    ...
    pub fn get_generic(&self) -> RawType {
        ...
        return self.get_inner_type().get_generic();
    }
    ...
    pub fn get_function(
        ...
        hash: u64,
    ) -> DiagnosticResult<(usize, TypedFunction)> {
        return match self.get_generic() {
            RawType::Struct(_, container) => Ok((
                container.get_function_hash(hash, storage)?,
                container.get_function(hash, storage)?,
            )),
            RawType::Enum(container) => Ok((
                container.get_function_hash(hash, storage)?,
                container.get_function(hash, storage)?,
            )),
            RawType::EnumEntry(container) => Ok((
                container.get_function_hash(hash, storage)?,
                container.get_function(hash, storage)?,
            )),
    ...
    pub fn get_field(
        ...
        storage: &TypedGlobalScope,
        hash: u64,
    ) -> DiagnosticResult<(usize, TypeReference)> {
        return match self.get_generic() {
            RawType::Struct(_, container) => Ok((
                container.get_field_hash(hash, storage)?,
                container.get_field(hash, storage)?,
            )),
            RawType::Enum(container) => Ok((
                container.get_field_hash(hash, storage)?,
                container.get_field(hash, storage)?,
    ...
    pub fn get_fields(&self, storage: &TypedGlobalScope) -> Vec<u64> {
        return match self.get_generic() {
    ...
    pub fn get_functions(&self, storage: &TypedGlobalScope) -> Vec<u64> {
        return match self.get_generic() {
