...
use crate::{TypedGlobalScope, tree::Type};
...
pub fn can_transmute_inner(array_type: &Type, new_type: &Type, storage: &TypedGlobalScope) -> bool {
    if !array_type.can_use_index_access() || !new_type.can_use_index_access() {
        panic!(
            "Either ones of the types sent when using can_transmute_inner were not actual array types!"
        )
    }

    return array_type
        .get_inner_type()
        .can_transmute(&new_type.get_inner_type(), storage);
}
