...
use crate::{
    SizedType, TypedGlobalScope,
    bounds::traits::Trait,
    enums::{RawEnumEntryContainer, RawEnumTypeContainer},
    structs::{LoweredStructTypeContainer, RawStructTypeContainer},
    tree::Type,
...
impl RawType {
    ...
    pub fn get_type_params_count<K: DiagnosticSpanOrigin>(
        &self,
        storage: &TypedGlobalScope,
        origin: &K,
    ) -> DiagnosticResult<usize> {
        match self {
            RawType::Enum(container) => Ok(container.type_params.len()),
            RawType::EnumEntry(container) => Ok(storage.entries[container.parent]
                .as_type(origin)?
                .get_type_params_count(storage, origin)?),
            RawType::Struct(_, container) => Ok(container.type_params.len()),
    ...
    pub fn has_trait(&self, t: Trait, raw_type: &Type) -> bool {
        match t {
            ...
            Trait::Numeric => {
                self.is_integer() || self.is_floating_point() || self.is_fixed_point()
            }
            Trait::CpuSupported => match self {
    ...
    fn get_size(&self, t: &Type, compacted_size: bool, storage: &TypedGlobalScope) -> usize {
        match self {
            ...
            RawType::LoweredStruct(_, container) => {
                return container.get_size(t, compacted_size, storage);
            }
