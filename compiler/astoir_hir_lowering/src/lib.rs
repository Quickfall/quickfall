...
pub fn lower_ast_body_node(
    ...
    match node.kind.clone() {
        ...
        ASTTreeNodeKind::VarValueChange { .. } => {
            return lower_ast_variable_assign(context, curr_ctx, node);
        }
        ASTTreeNodeKind::ArrayIndexModifiy { .. } => {
            return lower_ast_array_modify(context, curr_ctx, node);
        }

        ASTTreeNodeKind::MathResult { .. } => {
            return lower_ast_math_operation(context, curr_ctx, node, true);
        }
