...
use astoir_hir::{ctx::{HIRBranchedContext, HIRContext}, nodes::{HIRNode, HIRNodeKind}, structs::HIRIfBranch};
...
pub fn lower_ast_for_block(context: &mut HIRContext, curr_ctx: &mut HIRBranchedContext, node: Box<ASTTreeNode>) -> DiagnosticResult<Box<HIRNode>> {
	if let ASTTreeNodeKind::ForBlock { initial_state, cond, increment, body } = node.kind.clone() {
		...
		return Ok(Box::new(HIRNode::new(HIRNodeKind::ForBlock { initial_state:initial, condition, incrementation, body }, &node.start, &node.end)));
	}

	panic!("Invalid node passed!");
}

pub fn lower_ast_while_block(context: &mut HIRContext, curr_ctx: &mut HIRBranchedContext, node: Box<ASTTreeNode>) -> DiagnosticResult<Box<HIRNode>> {
	if let ASTTreeNodeKind::WhileBlock { cond, body } = node.kind.clone() {
		...
		return Ok(Box::new(HIRNode::new(HIRNodeKind::WhileBlock { condition, body }, &node.start, &node.end)));
	}

	panic!("Invalid node passed!");
}

pub fn lower_ast_if_statement_branch(context: &mut HIRContext, curr_ctx: &mut HIRBranchedContext, node: Box<ASTTreeNode>) -> DiagnosticResult<HIRIfBranch> {
	match node.kind {
...
pub fn lower_ast_if_statement(context: &mut HIRContext, curr_ctx: &mut HIRBranchedContext, node: Box<ASTTreeNode>) -> DiagnosticResult<Box<HIRNode>> {
	if let ASTTreeNodeKind::IfStatement { cond: _, body: _, branches, depth: _ } = node.kind.clone() {
		...
		return Ok(Box::new(HIRNode::new(HIRNodeKind::IfStatement { branches: hir_branches }, &node.end, &node.start)))	
	}
