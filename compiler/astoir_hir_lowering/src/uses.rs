...
use diagnostics::{
    ...
    builders::{
        make_cannot_find_type, make_diff_type_specifiers, make_req_type_kind, make_use_not_found,
    },
};
...
use crate::{
    ...
    types::{lower_ast_type, lower_sized_base_type},
};

pub fn handle_ast_use_statement_function_decl(
...
pub fn handle_ast_use_statement(
    context: &mut HIRContext,
    node: Box<ASTTreeNode>,
) -> MaybeDiagnostic {
    if let ASTTreeNodeKind::UseStatement {
        shards,
        use_clauses,
    } = node.kind.clone()
    {
...
pub fn gather_type_use<K: DiagnosticSpanOrigin>(
    context: &mut HIRContext,
    val: HashedString,
    origin: &K,
    pass: bool,
...
pub fn lower_ast_type_use_statement<K: DiagnosticSpanOrigin>(
    context: &mut HIRContext,
    t: ASTType,
    origin: &K,
    ctx: &ParserCtx,
