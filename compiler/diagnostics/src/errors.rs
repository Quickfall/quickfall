...
declare_error!(
    MISMATCHED_LITERAL,
    2,
    "expected literal of type {} but got {} instead"
);
declare_error!(
    MATH_OPERATION_ASSIGNS,
    3,
    "math operations here require assignments"
);
declare_error!(FIND_TYPE, 4, "cannot find type {} in the current scope");
declare_error!(
    EXPECTED_TYPE,
    5,
    "expected a value of type {} but got a value of type {} instead"
);
...
declare_error!(
    ENUM_PARENT_FIELDS,
    8,
    "fields are not supported in enum parents"
);
declare_error!(
    ENUM_CHILD_FUNCTIONS,
    9,
    "functions are not supported in enum children"
);
declare_error!(
    LAYOUT_FUNCTIONS,
    10,
...
declare_error!(
    TRAIT_MISSING,
    12,
    "bound trait {} cannot be applied onto {}"
);
declare_error!(BOUND_MISSING, 13, "bound {} cannot be applied onto {}");
declare_error!(
    VARIABLE_UNINIT,
    14,
    "variable {} must be initialized every usage"
);
declare_error!(TYPE_RESOLVE, 15, "type cannot be fully resolved here");
declare_error!(
    DIFF_SIZE_SPECIFIERS,
    16,
    "expected {} size specifiers on this type, got {}"
);
declare_error!(
    DIFF_TYPE_SPECIFIERS,
    18,
    "expected {} type specifiers on this type, got {}"
);
...
declare_error!(
    ASSIGN_DIFF_TYPE_IR,
    27,
    "cannot write on this value since the two types differ"
);
declare_error!(
    IR_DIFF_SSA_INDEXES,
    28,
    "cannot hint SSA val for pointers! indexes are different"
);
declare_error!(
    IR_INSTRUCTION_HELD_VAL,
    29,
    "cannot extract held value from instruction since it doesnt exist"
);
