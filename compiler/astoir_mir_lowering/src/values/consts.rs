...
pub fn lower_hir_literal(block: &mut MIRBlock, node: Box<HIRNode>, ctx: &MIRLoweringContext) -> BaseResult<BaseMIRValue> {
	match *node {
		HIRNode::IntegerLiteral { value, int_type } => {
			let t = &ctx.hir_ctx.type_storage.types[int_type];
			
			if t.is_signed() {
				let val = build_signed_int_const(&ctx.mir_ctx, block, value, t.get_size()?)?;

				return Ok(val.into());
			}

			let val = build_unsigned_int_const(&ctx.mir_ctx, block, value as u128, t.get_size()?)?;

			return Ok(val.into());
		},

		HIRNode::StringLiteral { value } => {
			let val = build_static_string_const(&ctx.mir_ctx, block, value)?;
