...
pub fn lower_hir_variable_reference_value(block: &mut MIRBlock, node: Box<HIRNode>, ctx: &MIRLoweringContext) -> BaseResult<BaseMIRValue> {
	let ptr = lower_hir_variable_reference(block, &node)?;

	let val = build_load(&ctx.mir_ctx, block, ptr)?;
	
	return Ok(val);
}

pub fn lower_hir_variable_assignment(block: &mut MIRBlock, node: Box<HIRNode>, ctx: &MIRLoweringContext) -> BaseResult<bool> {
	if let HIRNode::VarAssigment { variable, val } = *node {
		...
		build_store(&ctx.mir_ctx, block, ptr, val)?;
		return Ok(true);
	}
