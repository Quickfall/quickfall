...
use astoir_mir::{blocks::{MIRBlockVariableSSAHint, MIRBlockVariableType, refer::MIRBlockReference}, builder::{build_stack_alloc, build_store}};
use compiler_typing::SizedType;
use diagnostics::{MaybeDiagnostic};

use crate::{MIRLoweringContext, lower_hir_type, type_tools::cast_to_enum_child, values::lower_hir_value};

pub fn handle_var_introduction_queue(block: MIRBlockReference, node: Box<HIRNode>, ctx: &mut MIRLoweringContext) -> MaybeDiagnostic {
	if let HIRNodeKind::UnwrapCondition { original, new_type, new_var, unsafe_unwrap: _ } = node.kind.clone() {
